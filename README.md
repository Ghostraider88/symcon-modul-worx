...
Die Mower-Instanz stellt den bestätigten Zeitplan als natives Symcon-Wochenplanereignis „Mähzeitplan“ bereit. Das Ereignis ist der einzige Editor. Änderungen daran werden automatisch über MQTT an Worx gesendet; die Rückmeldung wird erst nach erneutem Lesen des passenden Gerätezeitplans als bestätigt angezeigt. `ApplyChanges()` selbst sendet keinen Zeitplan. Im Docker-Test wurden beide Richtungen bestätigt: Symcon-Änderungen erscheinen in der Worx-App und werden vom Mäher angenommen; Änderungen in der Worx-App erscheinen automatisch im Symcon-Ereignis, ohne manuellen Refresh. Beim Umschalten von „Ganzer Tag“ blieb der vom Modul redigiert angezeigte Worx-Datensatz unverändert. Ohne abweichendes Gerätefeld lässt sich diese App-Option nicht verlustfrei in Symcon abbilden; ein entsprechender Editorpunkt ist daher noch offen. Mähfenster über Mitternacht benötigen ebenfalls einen eigenen Datenbeleg.

### Weitere Geräteeinstellungen

Die Mower-Instanz bietet belegte Worx-Einstellungen wie automatischen Zeitplan, Regenverzögerung, tägliche Arbeitszeit, Sperre und — bei passender Capability — manuellen Kantenschnitt. Die verfügbaren Bedienelemente werden anhand der vom Mäher gemeldeten Fähigkeiten und Felder eingeblendet. Gesendete Einstellung und zurückgemeldeter Gerätezustand bleiben getrennt sichtbar; ein erfolgreicher Versand gilt noch nicht als Bestätigung. Die Firmware-Auto-Update-Option ändert nur die Cloud-Präferenz und startet selbst kein Firmware-Update. Die Schreib-Echos dieser Einstellungen sind auf dem WR105SI.1 noch nicht vollständig abgenommen; siehe [Release-Abnahme](docs/RELEASE_CHECKLIST.md).

Die bereitgestellten App-Bilder zeigen beim WR105SI.1 einen manuellen Wochenplan mit einem dargestellten Zeitfenster je Wochentag. In der Tagesansicht sind „Rasenkanten-Schnitt“, „Ganzer Tag“, Start, Ende und Löschen sichtbar. Außerdem ist „Automatischer Zeitplan“ ausgeschaltet und die Einstellung „Erhöhen/Verringern der täglichen Arbeitszeit“ steht auf 0 %. Die individuelle Anzeige ist nicht in den Quellcode übernommen. Der vom Nutzer bereitgestellte Gerätedatensatz meldet Firmware 3.52.0+1.
