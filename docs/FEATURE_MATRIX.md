...
- Docker-Testinstallation: Worx Cloud und Mower sind aktiv; das native Ereignis „Mähzeitplan“ ist Typ 2, deaktiviert und enthält sieben Tagesgruppen. `Schedule` und `SchedulePreview` sind verborgen. Ein früherer read-only JSON-RPC-Snapshot zeigte `cfg.sc.d` Montag `17:10`/`110` Minuten und das native Ereignis Typ 2 Montag `17:10` bis `19:00`; dieser Snapshot belegt nicht den späteren App→Symcon-Lauf. Der Nutzer hat bestätigt, dass Symcon-Änderungen in der Worx-App ankommen und der Mäher sie annimmt. Nach dem Update auf Commit `5809cdc` änderte der Nutzer den Zeitplan in der Worx-App; Symcon übernahm die Änderung sofort ohne manuellen Refresh. App→Ereignis ist damit im Docker-Test bestätigt.
...
| Manueller Wochenplan | Catomic hatte keinen Scheduler | App und redigierter WR105SI.1-Datensatz belegen Protokoll 0 und `cfg.sc.d` mit sieben Tagesfeldern. In Docker sind Typ 2, deaktivierter Zustand sowie die Übereinstimmung von `cfg.sc.d` und den nativen Ereignispunkten lesend verifiziert; der Status nennt die App-Übernahme. | Das native Symcon-Wochenplanereignis „Mähzeitplan“ ist der einzige Editor. Änderungen werden nach Abschluss der Symcon-Eventkaskade gebündelt und an MQTT `commandIn` gesendet; passendes `cfg.sc`-Echo bestätigt. | Ereignis→Mäher vom Nutzer mit App-Anzeige und Annahme bestätigt; App→Ereignis nach Commit `5809cdc` ohne manuellen Refresh vom Nutzer im Docker-Test bestätigt |
...
| Zeiterweiterung | Nicht vorhanden | App zeigt 0 %; Docker-Diagnose enthält `unrestricted_mowing_time` und `cfg.sc.p=0`. Vergleichsquelle beschreibt ein vorzeichenbehaftetes Prozentfeld; der App-Wertebereich ist nicht vollständig belegt. | Getrennte Eingabe „Zeiterweiterung setzen“ und bestätigte Istvariable, nur bei Capability und numerischem `p` im Bereich -100 bis 100. | Feld live gelesen; Schreibfunktion ergänzt; Geräteecho-Test offen |
| Regenverzögerung | Regenstatus vorhanden | Die redigierte Docker-Meldung des WR105SI.1 enthält `capabilities: rain_delay` und `cfg.rd: 30`. Die externe Worx-Integration führt `rd` als Minutenwert; die Bedeutung der aktuellen Gerätekonfiguration ist damit plausibel, der Schreibweg für genau dieses Gerät bleibt zu bestätigen. | Getrennte Eingabe „Regenverzögerung setzen“ und bestätigte Istvariable in Minuten; Capability- und Wertebereichsprüfung. | Feld live empfangen; Schreibfunktion ergänzt; Geräteecho-Test offen |
...
| 3 – Worx-Scheduler | Teilweise erledigt | Das native Ereignis ist vorhanden (sieben Tagesgruppen, inaktiv). Nach Nutzer-Retest auf der aktuellen Modulversion sind beide Richtungen bestätigt: Symcon→Worx wird in App/Mäher angenommen; Worx-App→Ereignis erscheint automatisch ohne manuellen Refresh. Die neue Anzeige „Symcon-Wochenplan geprüft“ stärkt die interne Rückleseprüfung. Offen bleiben „Ganzer Tag“/Mitternachtsgrenzen. Offen bleiben außerdem „Ganzer Tag“/Mitternachtsgrenzen. `ApplyChanges()` löst keinen Zeitplanversand aus. |
| 4 – Weitere Worx-Funktionen | In Arbeit | Regenverzögerung, Zeiterweiterung und Sperre haben capability-geprüfte getrennte Eingaben mit bestätigtem Istzustand; Nutzertest am WR105SI.1 steht aus. Weitere Bedienfelder benötigen konkrete Gerätebelege. |
...
Nächster Schritt: den automatischen Zeitplan-Schalter aus der neuen Version am WR105SI.1 einzeln testen und prüfen, ob der Cloud-Istwert nach dem Schalten zurückgelesen wird. Anschließend Regenverzögerung, Zeiterweiterung und Sperre einzeln testen; „Ganzer Tag“ und Mähfenster über Mitternacht brauchen weiter eigene Datenbelege. „Ganzer Tag“ und Mähfenster über Mitternacht bleiben gesondert zu belegen.
