...
| 3 – Worx-Scheduler | Teilweise erledigt | Das native Ereignis ist in Docker vorhanden (sieben Tagesgruppen, inaktiv), und dessen Status meldet eine aus der App übernommene Änderung. Der aktuelle lokale Branch bündelt manuelle Ereignisänderungen und sendet sie mit Rückbestätigung; diese neue Codefassung ist noch nicht installiert. Noch ausstehend: Ereignis→Mäher-Live-Test und Abgleich von „Ganzer Tag“/Mitternachtsgrenzen. `ApplyChanges()` löst keinen Zeitplanversand aus. |
