...
- Docker-Testinstallation: Worx Cloud und Mower sind aktiv; das native Ereignis „Mähzeitplan“ ist Typ 2, deaktiviert und enthält sieben Tagesgruppen. `Schedule` und `SchedulePreview` sind verborgen. Ein direkter read-only JSON-RPC-Abgleich des laufenden Docker-Kernels zeigt `cfg.sc.d` Montag `17:10`/`110` Minuten und das native Ereignis Typ 2 ebenfalls Montag `17:10` bis `19:00`; die anderen sechs Tagesgruppen stimmen ebenfalls überein. Der Status lautet „Änderung aus der Worx-App übernommen“. Damit ist App→Ereignis im gespeicherten Symcon-Objekt belegt. Der Nutzer bestätigt außerdem, dass Symcon-Änderungen in der Worx-App ankommen und der Mäher sie annimmt.
...
| Zeiterweiterung | Nicht vorhanden | App zeigt 0 %; Docker-Diagnose enthält `unrestricted_mowing_time` und `cfg.sc.p=0`. Vergleichsquelle beschreibt ein vorzeichenbehaftetes Prozentfeld; der App-Wertebereich ist nicht vollständig belegt. | Read-only-Variable „Zeiterweiterung“ mit Prozentprofil, nur bei Capability und numerischem `p` im Bereich -100 bis 100; Bedienung bis zum bestätigten Rundlauf aus. | Feldzuordnung live bestätigt; Anzeige ergänzt; Schreiben offen |
