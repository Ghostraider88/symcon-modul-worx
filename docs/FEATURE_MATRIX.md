...
| Manueller Wochenplan | Catomic hatte keinen Scheduler | App und redigierter WR105SI.1-Datensatz belegen Protokoll 0 und `cfg.sc.d` mit sieben Tagesfeldern | Das native Symcon-Wochenplanereignis „Mähzeitplan“ ist der einzige Editor. Ereignisänderung sendet `sc` auf MQTT `commandIn`; passendes `cfg.sc`-Echo bestätigt. | Implementiert; Live-Schreibbestätigung und Rücksynchronisierung einer App-Änderung in Docker noch zu prüfen |
