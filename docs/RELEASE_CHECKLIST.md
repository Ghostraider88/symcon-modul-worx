...
| Worx-App → Einstellungs-Eingaben in Symcon | Build 8 synchronisiert gemeldete Werte für Regenverzögerung, Zeiterweiterung und Sperre im Code; Unit-Test und CI erfolgreich. Docker-/Geräte-Rundlauf für diese drei Eingabefelder steht noch aus. |
...
| Aktueller Code-Stand im Testbranch | Build 10 ergänzt den Rückfall abgelehnter oder nicht bestätigter Sollwerte auf zuletzt bestätigte Gerätezustände. Für Runtime-Code und vorherige Tests sind GitHub-Tests, Style- und PHP-8.5-Syntaxprüfung für Commit `4d1f4cd` erfolgreich. Der aktuelle HEAD `a8bb143` ergänzt Tests zur Cloud-/App-Synchronisierung des automatischen Zeitplans; dessen CI-Ergebnis ist noch nicht bestätigt. Build 10 ist laut letztem Docker-Leseabgleich noch nicht installiert (dort Build 7). |
