...
`ForkIdentityTest` sichert die stabilen, eigenständigen Library- und Modul-GUIDs gegenüber dem Catomic-Basiscommit ab. `WorxScheduleCodecTest` prüft den Worx-Protokoll-0-Zeitplan, die Abbildung ins native Symcon-Wochenplanereignis, Wertegrenzen und den Erhalt unbekannter Cloud-Felder. `ValidationTest` prüft Library- und Modulmanifestdateien. `WorxCloudSecurityTest` stellt sicher, dass Seriennummern in API-Debugpfaden maskiert werden.
